...
import { TUserSecret, useGetUserSecrets } from "@app/hooks/api/userSecret";
    ...
    <TableContainer>
      <Table>
        ...
        <TBody>
          ...
          {!isLoading &&
            data?.secrets?.map((row: TUserSecret) => (
      ...
      {!isLoading && !data?.secrets?.length && (
        <EmptyState title="No user web login secrete yet" icon={faKey} />
      )}
    </TableContainer>
  );
};
